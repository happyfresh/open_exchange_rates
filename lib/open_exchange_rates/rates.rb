...
module OpenExchangeRates
  class Rates
    ...
    def parse_latest
      JSON.parse(open("#{OpenExchangeRates::LATEST_URL}?app_id=#{@app_id}").read)
    end

    def parse_on(date_string)
      JSON.parse(open("#{OpenExchangeRates::BASE_URL}/historical/#{date_string}.json?app_id=#{@app_id}").read)
    end
